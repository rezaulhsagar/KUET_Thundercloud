...
struct Treap{
    ...
    inline void meld (node* &cur, node* l, node* r){
        if (!l or !r) cur = l ? l : r;
        else if (l->priority > r->priority) meld (l->r, l->r, r), cur = l;
        else meld (r->l, l, r->l), cur = r;
        if (cur) cur->update();
    }
    ...
    inline void erase (node* &cur, int key){
        if (!cur) return;
        if (cur->key == key) meld (cur, cur->l, cur->r);
